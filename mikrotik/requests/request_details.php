        ...
        function refresh_page_data() {
            $.post("<?= $api_url ?>requests/request_details_api.php"
                ...
                if (data.error != true) {
                    ...
                    //Show refund first payment option only if terminate action after first installation
                    $(".refund-first-payment").hide();
                    if (data.request_order_row.cable_subscriber === "yes"){
                        if(data.request_row.action == "terminate" && data.request_order_row.cancellation_date < data.request_row.action_on_date){
                            alert(data.request_row.action_on_date);
                            $(".refund-first-payment").show();
                        }
                    }
                    else{
                        if(data.request_row.action == "terminate" && data.request_order_row.installation_date_1 < data.request_row.action_on_date){
                            $(".refund-first-payment").show();
                        }
                    }
                }
...
///////// form submit ajax call
        $("#no_verdict").submit(function (e) {
            e.preventDefault();
            var refund_first_payment = $("#rfpcheckbox").prop('checked'); //refund first payment
            if(refund_first_payment){
                refund_first_payment = 1;
            }
            else{
                refund_first_payment = 0;
            }
            ...
            $.post("<?= $api_url ?>requests/request_details_api.php"
                    , {
                        ...
                        "refund_first_payment": refund_first_payment,
...
<form id="no_verdict" class="register-form" >
    ...
    <div class="form-group">
        ...
        <div class="form-group refund-first-payment">
            <label>Refund First Payment:</label>
            <input type="checkbox" name="refund_first_payment" id="rfpcheckbox" value="1"/>
        </div>
    </div>
