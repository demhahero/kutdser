...
$invoice_info_result = $dbTools->query(
  "SELECT
    ...
    WHERE `invoices`.`order_id`={$order_id} AND `invoices`.`invoice_type_id`=1");

$invoice_info= $dbTools->fetch_assoc($invoice_info_result);
...
$sub_total_price=0;
$total_price=0;
$subtotal_items = $dbTools->query($subtotal_items_query);
while($subtotal_item=$dbTools->fetch_assoc($subtotal_items))
{
  $sub_total_price=$subtotal_item["subtotal"];
  array_push($invoice_items,$subtotal_item);
}
$tax_items = $dbTools->query($tax_items_query);
while($tax_item=$dbTools->fetch_assoc($tax_items))
{
