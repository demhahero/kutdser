...
left JOIN `customers` resellers on resellers.`customer_id` = `customers`.`reseller_id` where product_title LIKE '%discount%' ";
...
while ($row = mysqli_fetch_array($queryRecords)) {
    ...
    $data[5] = $row['status'];
    $data[6] = '<a target="_blank" href="edit_last_recurring_price.php?order_id='
            . $row['order_id'] . '" class="btn btn-primary btn-xs"><i class="fa fa-edit"></i> Edit </a>';
    $data[7] = '<a target="_blank" href="' . $api_url . 'print/print_order_test.php?order_id='
