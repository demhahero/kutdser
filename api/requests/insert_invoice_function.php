...
function terminateMonthly($dbTools, $postData) {
    ...
    if($postData["refund_first_payment"] == "1"){
        $previous_invoice_query = "SELECT *,DATEDIFF(`valid_date_to`,`valid_date_from`) AS `duration` FROM `invoices` WHERE `invoice_type_id` in (1,2,3) AND `customer_id`=? AND `order_id`=? ORDER BY `valid_date_from` DESC LIMIT 1";
        $stmt1 = $dbTools->getConnection()->prepare($previous_invoice_query);
        $stmt1->bind_param('ss', $customer_id,$order_id);
        $stmt1->execute();
        ...
        if ($stmt3->insert_id > 0) {
            ...
            $query = "SELECT "
                    ...
                    . "FROM `orders` LEFT JOIN `order_options` "
                    . "ON `order_options`.`order_id`= `orders`.`order_id` "
            ...
            $total = $result["total_price"] - $result["gst_tax"] - $result["qst_price"];
            
            $param1 = $invoice_id;
            $param2 = "Refund for terminate before installation";
            $param3 = $total;
            $param4 = "QST Tax";
            $param5 = $result["qst_tax"];
            $param6 = "GST Tax";
            $param7 = $result["gst_tax"];
            $stmt4 = $dbTools->getConnection()->prepare($invoice_item_query);
