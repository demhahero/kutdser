...
while ($row = mysqli_fetch_array($queryRecords)) {
  ...
  if ((int) $row["order_id"] > 10380)
    ...
    $data[0] = '<a href="'.$site_url.'/orders/order_details.php?order_id='.$row["order_id"].'">
        '.$row["displayed_order_id"].'
    </a>';
    $data[1] = $row["product_title"];
    $data[2] = $row['creation_date'];
    $data[3] = '<a href="'.$site_url.'/requests/make_request.php?order_id='.$row['order_id'].'">Make request</a>';
