...
$orders = $dbTools->order_query_api("SELECT `order_options`.`cable_subscriber`,
    `orders`.order_id,`orders`.creation_date,`orders`.status,`orders`.reseller_id,`orders`.customer_id,
...
where `orders`.creation_date >= '".$_GET['date1']."' and `orders`.creation_date <= '".$_GET['date2']."' 
    and `order_options`.`cable_subscriber`='".$_GET['cable_subscriber']."'
