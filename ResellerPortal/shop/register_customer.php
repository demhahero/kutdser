...
if (isset($_POST["full_name"])) {
    ...
    if (isset($_POST["options"])) {
        ...
        /// if it is tv order insert add on channels
        if ($result_order_options && isset($options["add_on_channels"])) {
            foreach ($options["add_on_channels"] as $key => $channel) {
                $query = "INSERT INTO `tv_order_channels`
                ( `order_id`, `tv_channel_id`, `tv_channel_text`, `tv_channel_price`)
                 VALUES
                 ( N'" . $order_id . "', N'" . $channel["id"] . "', N'" . $channel["text"] . "', N'" . $channel["price"] . "')";
                $dbToolsReseller->query($query);
            }
        }
    ...
    if ($customer_id > 0 && $order_id > 0 && $result_order_options && $result_merchantrefs) {
        ...
        if ($dbToolsReseller->query($invoice_item_query) === TRUE) {
            ...
            echo $invoice_id . "_" . (((0x0000FFFF & (int) $order_id) << 16) + ((0xFFFF0000 & (int) $order_id) >> 16));

            try {
                $printOrder = new PrintOrder();
                file_put_contents('last_order.pdf', $printOrder->outputInvoice($invoice_id));
