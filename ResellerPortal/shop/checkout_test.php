...
if ($product_type == "internet") {
    ...
    $subscription_recurring_amount = $product_price;
      ...
      //If rent modem
      if (isset($_POST["options"]["box"]) && $_POST["options"]["box"] == "yes") {
          $box_price = 50;
          if($has_discount && $free_adapter)
          $box_price=0;
      }
      $add_on_channels=[];
      foreach ($_POST["options"]["add_on_channels"] as $key => $value) {
        // code...
        $json = $value;
        $add_on_channel=json_decode($json, true);
        $add_on_channels_price+=$add_on_channel["price"];
        $add_on_channel["remaining_days_price"]=0;
        array_push($add_on_channels,$add_on_channel);
      }
      $_POST["options"]["add_on_channels"]=$add_on_channels;
      //NOTICE: Have to be changed later
      //$adapter_cost = 0;

      //If transfer
      if (isset($_POST["options"]["admin_fee"]) && $_POST["options"]["admin_fee"] == "yes") {
          $admin_fee_price = (double)$_POST["options"]["admin_fee_value"] ;
      }
      ...
      if ((int) $start_date->format('d') == 1) {
          ...
          if ($subscription_period_type == "YEARLY") { //if yearly payment, divide price by 12 months
              $price_of_remaining_days = (($product_price / 12) / $days_in_month) * $remainingDays;
              $product_duration_price=((($product_price / 12) / $days_in_month) * $remainingDays);
              foreach ($add_on_channels as $key => $add_on_channel) {
                $add_on_channel["remaining_days_price"]=((($add_on_channel["price"] / 12) / $days_in_month) * $remainingDays);
                $price_of_remaining_days_channels+=$add_on_channel["remaining_days_price"];
                $add_on_channels[$key]=$add_on_channel;
              }
          } else { // if monthly payment
              $price_of_remaining_days = ($product_price / $days_in_month) * $remainingDays;
              $product_duration_price=(($product_price / $days_in_month) * $remainingDays);
              foreach ($add_on_channels as $key => $add_on_channel) {
                $add_on_channel["remaining_days_price"]=(($add_on_channel["price"]/ $days_in_month) * $remainingDays);
                $price_of_remaining_days_channels+=$add_on_channel["remaining_days_price"];
                $add_on_channels[$key]=$add_on_channel;
              }
          }
      ...
      //Calculate total price
      $total_price = $product_price + $price_of_remaining_days+$price_of_remaining_days_channels+$add_on_channels_price + $box_price + $admin_fee_price;
      ...
      $_POST["options"]["product_price"] = $product_price;
      $_POST["options"]["remaining_days_price"] = $price_of_remaining_days;
      $_POST["options"]["admin_fee_price"] = $admin_fee_price;
      $_POST["options"]["box_price"] = $box_price;
      $_POST["options"]["total_price"] = $total_price;
      $_POST["options"]["qst_tax"] = $qst_tax;
      $_POST["options"]["gst_tax"] = $gst_tax;
      ...
      $_POST["invoice_items"][]=["item_name"=>"Product ".$row_product["title"]." With remaining ".$remainingDays." days","item_price"=>$product_price,"item_duration_price"=>($product_price+$product_duration_price),"item_type"=>"duration"];
      $_POST["invoice_items"][]=["item_name"=>"Box price","item_price"=> $box_price,"item_duration_price"=> $box_price,"item_type"=>"once"];
      $_POST["invoice_items"][]=["item_name"=>"Admin Fee Price","item_price"=> $admin_fee_price,"item_duration_price"=> $admin_fee_price,"item_type"=>"once"];
      $_POST["invoice_items"][]=["item_name"=>"QST tax","item_price"=> $qst_tax,"item_duration_price"=> $qst_tax,"item_type"=>"once"];
      $_POST["invoice_items"][]=["item_name"=>"GST tax","item_price"=> $gst_tax,"item_duration_price"=> $gst_tax,"item_type"=>"once"];

      foreach ($add_on_channels as $key => $add_on_channel) {
        $_POST["invoice_items"][]=["item_name"=>$add_on_channel["text"],"item_price"=>$add_on_channel["price"],"item_duration_price"=>($add_on_channel["price"]+$add_on_channel["remaining_days_price"]),"item_type"=>"duration"];

      }

      $subscription_recurring_amount = $product_price+$add_on_channels_price;
  }
...
// echo "address_line_1: ".  $_POST["address_line_1"] ."</br>";
// echo "address_line_2: ". $_POST["address_line_2"] ."</br>";
// echo "postal_code: ". $_POST["postal_code"] ."</br>";
// echo "city:  ". $_POST["city"] ."</br>";
// echo "email:  ".  $_POST["email"] ."</br>";
// echo "phone:  ".  $_POST["phone"] ."</br>";
// echo "note:  ".  $_POST["note"] ."</br>";
// echo "customer_id:  ".  $_POST["customer_id"] ."</br>";
// echo "start_active_date:  ". $start_active_date_string."</br>";
// echo "recurring_date:   ".  $subscription_start_date ."</br>";
// echo "recurring_amount:   ".  $subscription_recurring_amount ."</br>";
// echo "merchantref:   ".  $merchantref ."</br>";
// echo "options: ";
// print_r($_POST["options"]);
// echo "invoice_items:";
// print_r($_POST["invoice_items"]);
// exit();
